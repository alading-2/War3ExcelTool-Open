...
def parse_args():
    ...
    parser.add_argument(
        "-r",
        "--recursive",
        # * 注意：store_false：不填参数：默认true，填参数：false
        action="store_false", # 默认true
...
def main():
    ...
    if is_cli_mode: 
        ...
        # --- 为处理器构建配置字典 --- 
        config = {
            # 路径 os.path.normpath 规范化每个文件路径，比如把'else/Test/excel\\test\\xls格式.xls'都替换成\
