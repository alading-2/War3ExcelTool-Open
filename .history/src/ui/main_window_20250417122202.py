...
class MainWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        options_layout.addWidget(self.convert_ini_to_excel_check, 1, 1) # 第1行，第1列
        options_layout.addWidget(self.recursive_check, 2, 0, 1, 2) # 第2行，第0列，跨2列
    ...
    def start_conversion(self):
        ...
        config = {
            # --- 核心路径 --- 
            "input_path": self.input_dir_edit.text().strip(), # Excel 输入路径 (目录)
    ...
    def log_message(self, message: str):
        ...
        if message.startswith("成功:"):
            colored_message = f'<font color="green">{message}</font>'
        elif message.startswith("错误:"):
            colored_message = f'<font color="red">{message}</font>'
        elif message.startswith("跳过:"):
            colored_message = f'<font color="orange">{message}</font>'
        elif message.startswith("警告:"):
            colored_message = f'<font color="orange">{message}</font>' # 警告也用橙色
        else:
            colored_message = message # 其他消息使用默认颜色

        self.log_text.appendHtml(colored_message) # 使用 appendHtml 追加带格式的文本
