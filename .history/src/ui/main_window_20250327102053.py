...
from ..core.type_handler import TypeHandler
        ...
        try:
            ...
            for i, file_path in enumerate(excel_files):
                try:
                    ...
                    # 解析Excel
                    metadata, keys, data_rows, color_map = self.excel_parser.parse_excel(file_path)
                    
                    # 生成TS文件
                    output_file = self.ts_generator.generate_ts_file(
                        excel_name, metadata, keys, data_rows, color_map, self.output_dir
                    )
            ...
            # 完成
            if successful_conversions == len(excel_files):
                self.conversion_done.emit(True, f"成功转换了所有 {len(excel_files)} 个文件")
            else:
                self.conversion_done.emit(
                    True, 
                    f"完成转换: {successful_conversions}/{len(excel_files)} 个文件成功"
                )
