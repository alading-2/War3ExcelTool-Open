...
class MainWindow(QMainWindow):
    ...
    def show_about_dialog(self):
        ...
        QMessageBox.about(
            self,  # 父窗口
            f"关于{project_info.name}(Excel转ts、lua、ini，ini转excel)",  # 对话框标题
            # 对话框内容文本
            f"{project_info.name} v{project_info.version}\n\n"
            f"功能：Excel转ts、lua、ini，ini转excel。\n\n"
            f"作者：{project_info.author}",
        )
