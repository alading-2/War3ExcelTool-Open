from PyQt5.QtWidgets import (
    QDialog, QVBoxLayout, QHBoxLayout, QFormLayout,
    QLabel, QLineEdit, QComboBox, QSpinBox,
    QPushButton, QDialogButtonBox, QTabWidget, QGroupBox,
    QGridLayout, QCheckBox, QWidget
)
from PyQt5.QtCore import Qt

from src.utils.json_config import ConfigManager

class ConfigDialog(QDialog):
    """配置对话框，用于高级设置"""

    def __init__(self, config_manager: ConfigManager, parent=None):
        """
        初始化配置对话框
        
        Args:
            config_manager: 配置管理器实例
            parent: 父窗口
        """
        super().__init__(parent)
        
        self.config_manager = config_manager
        
        self.setWindowTitle("高级配置")
        self.setMinimumWidth(500)
        self.setMinimumHeight(400)
        
        self.init_ui()
        self.load_config()
    
    def init_ui(self):
        """初始化用户界面"""
        layout = QVBoxLayout()
        self.setLayout(layout)
        
        # 创建选项卡控件
        tab_widget = QTabWidget()
        layout.addWidget(tab_widget)
        
        # --- 常规设置选项卡 ---
        general_tab = QWidget()
        general_layout = QVBoxLayout()
        general_tab.setLayout(general_layout)
        
        # 常规设置表单
        general_form = QFormLayout()
        
        # 日志级别
        self.log_level_combo = QComboBox()
        self.log_level_combo.addItems(["DEBUG", "INFO", "WARNING", "ERROR", "CRITICAL"])
        general_form.addRow("日志级别:", self.log_level_combo)
        
        # 添加表单到布局
        general_layout.addLayout(general_form)
        
        # --- 路径设置选项卡 ---
        paths_tab = QWidget()
        paths_layout = QVBoxLayout()
        paths_tab.setLayout(paths_layout)
        
        # 路径设置表单
        paths_form = QFormLayout()
        
        # 生成INI路径
        self.ini_output_edit = QLineEdit()
        paths_form.addRow("INI输出路径:", self.ini_output_edit)
        
        # w3x2lni路径
        self.w3x2lni_path_edit = QLineEdit()
        paths_form.addRow("w3x2lni工具路径:", self.w3x2lni_path_edit)
        
        # 添加表单到布局
        paths_layout.addLayout(paths_form)
        
        # --- Excel选项卡 ---
        excel_tab = QWidget()
        excel_layout = QVBoxLayout()
        excel_tab.setLayout(excel_layout)
        
        # Excel设置表单
        excel_form = QFormLayout()
        
        # 按字母排序选项
        self.sort_by_alpha_check = QCheckBox("按字母排序生成表格")
        excel_form.addRow("", self.sort_by_alpha_check)
        
        # 字体大小
        self.font_size_spin = QSpinBox()
        self.font_size_spin.setRange(8, 36)
        excel_form.addRow("字体大小:", self.font_size_spin)
        
        # 添加表单到布局
        excel_layout.addLayout(excel_form)
        
        # --- 添加所有选项卡 ---
        tab_widget.addTab(general_tab, "常规设置")
        tab_widget.addTab(paths_tab, "路径设置")
        tab_widget.addTab(excel_tab, "Excel设置")
        
        # 按钮
        button_box = QDialogButtonBox(
            QDialogButtonBox.Ok | QDialogButtonBox.Cancel | QDialogButtonBox.Reset
        )
        button_box.accepted.connect(self.accept)
        button_box.rejected.connect(self.reject)
        button_box.button(QDialogButtonBox.Reset).clicked.connect(self.reset_to_default)
        
        layout.addWidget(button_box)
    
    def load_config(self):
        """从配置中加载设置"""
        # 常规设置
        log_level = self.config_manager.get("log_level", "INFO")
        index = self.log_level_combo.findText(log_level)
        if index >= 0:
            self.log_level_combo.setCurrentIndex(index)
        
        # 路径设置
        self.ini_output_edit.setText(self.config_manager.get("ini_output", ""))
        self.w3x2lni_path_edit.setText(self.config_manager.get("w3x2lni_path", ""))
        
        # Excel设置
        self.sort_by_alpha_check.setChecked(self.config_manager.get("sort_by_alpha", False))
        self.font_size_spin.setValue(self.config_manager.get("font_size", 14))
    
    def accept(self):
        """确认按钮处理"""
        # 保存常规设置
        self.config_manager.set("log_level", self.log_level_combo.currentText())
        
        # 保存路径设置
        self.config_manager.set("ini_output", self.ini_output_edit.text())
        self.config_manager.set("w3x2lni_path", self.w3x2lni_path_edit.text())
        
        # 保存Excel设置
        self.config_manager.set("sort_by_alpha", self.sort_by_alpha_check.isChecked())
        self.config_manager.set("font_size", self.font_size_spin.value())
        
        # 保存所有配置到文件
        self.config_manager.save_config()
        
        super().accept()
    
    def reset_to_default(self):
        """重置所有设置为默认值"""
        self.config_manager.reset_to_default()
        self.load_config() 