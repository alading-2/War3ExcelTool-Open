...
from PyQt5.QtWidgets import (
    ...
    QProgressBar, QTextEdit, QMessageBox, QGroupBox, QAction, QMenu
)
...
class ConversionWorker(QThread):
    ...
    def __init__(self, input_dir: str, output_dir: str, recursive: bool):
        ...
        self.ts_generator = TSGenerator(self.type_handler)
        ...
        try:
            ...
            for i, file_path in enumerate(excel_files):
                try:
                    ...
                    # 解析Excel的所有sheet
                    sheets_data = self.excel_parser.parse_excel_sheets(file_path)
                    
                    # 如果只有一个sheet，省略sheet名
                    if len(sheets_data) == 1:
                        sheet_name, metadata, keys, data_rows, color_map = sheets_data[0]
                        
                        # 生成TS文件
                        output_file = self.ts_generator.generate_ts_file(
                            excel_name, metadata, keys, data_rows, color_map, target_dir
                        )
                        ...
                        for sheet_name, metadata, keys, data_rows, color_map in sheets_data:
                            ...
                            try:
                                # 生成TS文件
                                output_file = self.ts_generator.generate_ts_file(
                                    sheet_excel_name, metadata, keys, data_rows, color_map, target_dir,
                                    sheet_name=sheet_name  # 传递sheet名便于日志记录
                                )
                                
                                self.log_message.emit(f"成功生成({sheet_name}): {output_file}")
                                successful_conversions += 1
                            except Exception as sheet_e:
                                self.log_message.emit(f"处理sheet {sheet_name} 时出错: {str(sheet_e)}")
            ...
            if successful_conversions == total_conversions:
                self.conversion_done.emit(True, f"成功转换了所有 {total_conversions} 个工作表")
            else:
