...
from src.utils.json_config import JsonConfigManager  # 导入新的JSON配置管理类
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        # 设置窗口的基本属性
        self.setWindowTitle("Excel到TypeScript转换工具")
    ...
    def start_conversion(self):
        ...
        config = {
            # --- 核心路径 --- 
            "input_path": os.path.normpath(self.input_dir_edit.text().strip()), # Excel 输入路径 (目录)
            "output_path": os.path.normpath(self.output_dir_edit.text().strip()), # 基础输出路径 (目录)
            # --- 转换选项 --- 
    ...
    def show_about_dialog(self):
        ...
        QMessageBox.about(
            self, # 父窗口
            "关于Excel到TypeScript转换工具", # 对话框标题
            # 对话框内容文本
            "Excel到TypeScript转换工具 v1.0.0\n\n"
            "功能：将Excel表格数据自动转换为TypeScript代码，支持类型推断和复杂数据结构。\n\n"
            "作者：开发团队"
        )
