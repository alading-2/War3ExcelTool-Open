...
class MainWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        # main_layout.addWidget(self.ini_input_group) # Removed INI input group
    ...
    def start_conversion(self):
        ...
        config = {
            # --- 核心路径 --- 
            "input_path": self.input_dir_edit.text().strip(), # 统一输入路径 (目录或文件)
            "output_path": self.output_dir_edit.text().strip(), # 基础输出路径 (目录)
            # "ini_input_path": self.ini_input_edit.text().strip(), # Removed
            # --- 转换选项 --- 
            "recursive": self.recursive_check.isChecked(), # 是否递归处理输入目录
            "convert_to_ts": self.convert_to_ts_check.isChecked(), # 是否执行 Excel -> TS
            "convert_to_lua": self.convert_to_lua_check.isChecked(), # 是否执行 Excel -> Lua
            "convert_to_ini": self.convert_to_ini_check.isChecked(), # 是否执行 Excel -> Ini
            "convert_ini_to_excel": self.convert_ini_to_excel_check.isChecked(), # 是否执行 Ini -> Excel
            # --- 高级设置 (从 ConfigManager 获取) --- 
            "log_level": self.config_manager.get("log_level", "INFO"), # 日志级别
            "ts_template_path": self.config_manager.get("ts_template_path", ""), # TS 模板文件路径
            "lua_template_path": self.config_manager.get("lua_template_path", ""), # Lua 模板文件路径
            "ini_section_style": self.config_manager.get("ini_section_style", "SheetName"), # Ini 文件 Section 生成方式
            "ini_comment_style": self.config_manager.get("ini_comment_style", "Both"), # Ini 文件注释风格
            "excel_header_row": self.config_manager.get("excel_header_row", 1), # Excel 标题行号 (1-based)
            "excel_data_start_row": self.config_manager.get("excel_data_start_row", 2), # Excel 数据起始行号 (1-based)
            "excel_comment_column": self.config_manager.get("excel_comment_column", "A"), # Excel 注释列字母
            "excel_key_column": self.config_manager.get("excel_key_column", "B"), # Excel 键列字母
            "excel_value_column": self.config_manager.get("excel_value_column", "C"), # Excel 值列字母
        }
        ...
        # 3. 如果进行 Ini -> Excel 转换，必须指定存在的输入路径 (文件或目录)
        if ini_to_excel:
            if not config["input_path"]: # 使用统一的 input_path
                 QMessageBox.warning(self, "缺少信息", "请为 Ini->Excel 转换指定输入路径")
                 return
            if not os.path.exists(config["input_path"]):
                 QMessageBox.warning(self, "输入错误", f"输入路径不存在: {config['input_path']}")
                 return
        ...
        self.worker.conversion_done.connect(self.conversion_finished) # 连接转换完成信号
        # 启动线程
        self.worker.start()
    ...
    def load_config(self):
        ...
        # 根据加载的 Ini->Excel 选项，切换INI输入组的可见性 (已移除)
        # self.toggle_ini_input_visibility()
        # 更新最近文件菜单 (如果实现了该功能)
        # self.update_recent_files_menu() # 更新最近文件菜单

    def save_config(self):
        """
        将当前UI控件中的设置保存到配置管理器，并写入配置文件。
        - 从 QLineEdit 和 QCheckBox 获取当前值。
        - 使用 config_manager.set 将值存入配置管理器的内存中。
        - 调用 config_manager.save_config() 将内存中的更改持久化到文件。
        """
        # 保存 Excel 输入目录
        ...
        # 将当前输入路径添加到最近文件列表 (如果实现了该功能)
        excel_input = self.input_dir_edit.text()
        # ini_input = self.ini_input_edit.text() # Removed
        # 如果有 Excel 输入:
        #     self.config_manager.add_recent_file(excel_input) # 添加到最近文件列表
        # 如果有 Ini 输入 (已移除):
