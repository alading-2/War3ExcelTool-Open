...
from PyQt5.QtWidgets import (QApplication, QMainWindow, QWidget, QVBoxLayout,
                             ...
                             QMessageBox, QGroupBox, QAction, QMenu)
        ...
        try:
            ...
            for i, file_path in enumerate(excel_files):
                try:
                    ...
                    if len(sheets_data) == 1:
                        sheet_name, metadata, keys, data_rows, color_map = sheets_data[
                            0]

                        # 生成TS文件
                        output_file = self.ts_generator.generate_ts_file(
                            excel_name, metadata, keys, data_rows, color_map,
                            target_dir)
                        ...
                        for sheet_name, metadata, keys, data_rows, color_map in sheets_data:
                            ...
                            safe_sheet_name = self._sanitize_sheet_name(
                                sheet_name)
            ...
            if successful_conversions == total_conversions:
                self.conversion_done.emit(True,
                                          f"成功转换了所有 {total_conversions} 个工作表")
            else:
...
class MainWindow(QMainWindow):
    ...
    def show_about_dialog(self):
        """显示关于对话框"""
        QMessageBox.about(
            self, "关于Excel到TypeScript转换工具", "Excel到TypeScript转换工具 v1.0.0\n\n"
    ...
    def load_config(self):
        ...
        self.recursive_check.setChecked(
            self.config_manager.get("recursive", False))
