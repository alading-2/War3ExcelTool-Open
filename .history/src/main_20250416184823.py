...
def main():
    ...
    # 任何转换相关的参数都会触发CLI模式。
    is_cli_mode = bool(
        args.input or # Excel输入 
        args.output or # 输出是CLI模式必需的
        args.ini_input or # Ini输入 
        args.to_ts or # 转TS 
        args.to_lua or # 转Lua 
        args.to_ini or # 转Ini 
        args.ini_to_excel # Ini转Excel
    )

    # --- 根据模式执行 --- 
    if is_cli_mode: # 如果是CLI模式
        logger.info("在命令行界面 (CLI) 模式下运行。")
        closesplash() # CLI模式立即关闭启动画面
        ...
        # --- GUI 模式 --- 
        logger.info("未提供特定的转换参数，尝试在GUI模式下运行。")
        
        if QApplication is None or MainWindow is None: # 检查GUI组件是否可用
            logger.error("未找到或导入GUI组件 (PyQt5) 失败。无法启动GUI模式。")
            logger.error("请安装 PyQt5: pip install PyQt5")
            sys.exit(1) # 退出程序

        closesplash() # 仅在确认启动GUI时关闭启动画面
