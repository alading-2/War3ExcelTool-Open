...
from .ini.excel_to_ini_generate import Excel_to_ini  # Ini生成器
...
def process_files():
    ...
    for excel_file in excel_files:
        try:
            ...
            excel_data = excel_parser.parse_excel(excel_file)
            ...
            if convert_to_ts:
                ...
                ts_generator.generate_ts_file(excel_name, excel_data,
                                              ts_output_dir)
            ...
            if convert_to_ini:
                ...
                try:
                    ...
                    ini_output_dir = os.path.join(
                        base_output_dir, "output", "ini",
                        os.path.dirname(relative_path))
                    FileUtils.ensure_directory(ini_output_dir)
                    # 创建ExcelToIniGenerator实例，传入config_manager
                    ini_generator = ExcelToIniGenerator(config_manager)
                    # 处理当前excel文件的数据
                    ini_generator.process_excel_to_ini(excel_data, excel_file)
                    # 输出ini和ts映射文件
                    error_msgs = ini_generator.finish_processing(
                        ini_output_dir)
                    if error_msgs:
                        logger.warning(f"Excel转INI时有警告/错误: {error_msgs}")
                    logger.info(f"成功生成INI及TS映射文件，输出目录: {ini_output_dir}")
                except Exception as e:
                    logger.error(f"Excel转INI处理失败: {e}", exc_info=True)
                    results["errors"] += 1
                    results["details"].append({
                        "file": excel_file,
                        "status": "错误",
                        "reason": f"Excel转INI处理失败: {e}"
                    })
            ...
            if w3x_files:
                ...
                for w3x_file in w3x_files:
                    ...
                    temp_output_dir = os.path.join(base_output_dir, "output",
                                                   "w3x", f"{w3x_basename}")
                    FileUtils.ensure_directory(temp_output_dir)
                    w3x_parse_paths.append(temp_output_dir)
                    # 处理w3x文件
                    try:
                        # 解析w3x文件
    ...
    # --- Ini 到 Excel 的转换 ---
    # 使用统一的 input_path
    def ini_to_excel(path, excel_name="物编"):
        if convert_ini_to_excel:
            logger.info(f"正在扫描 Ini 文件于: {path}")
            ini_files = FileUtils.find_files(
                path, recursive, ("*.ini", ))  # 使用 input_path 查找 ini 文件
            ...
            if not ini_files:
                ...
                # 确保输出目录存在
                excel_output_dir = os.path.join(base_output_dir, "output",
                                                "excel")
                ...
                for ini_file in ini_files:
                    try:
                        ...
                        ini_type = War3IniParser.detect_ini_type(
                            file_name)  # 检测Ini类型 - 使用类方法而非实例方法

                        if not ini_type:
                            string = f"无法识别 {ini_file} 的物编类型，跳过,仅支持这些类型: {OBJECT_TYPE_MAPPING.keys()}。"
                ...
                if ini_data_dict:
                    try:
                        ...
                        if success:
                            logger.info(
                                f"成功将Ini数据写入Excel: {excel_output_path}")
                        else:
