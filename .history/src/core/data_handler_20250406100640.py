...
import traceback
from src.core.excel_parser import infer_types, _value_handle  # 导入已存在的ColumnInfo类
...
class ExcelTable:
    ...
    def excel_table(self, raw_df: pd.DataFrame):
        """
        ...
        try:
            ...
            # 获取列类型
            types = infer_types(self._data)
            self.types = types

            # 对DataFrame所有数据进行处理
            self._data[2:] = self._data[2:].apply(_value_handle)
