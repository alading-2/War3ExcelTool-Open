...
def get_colors_from_excel(file_path, sheet_name, cell_ref):
    ...
    try:
        with zipfile.ZipFile(file_path) as zip_file:
            ...
            if not sheet_id:
                raise ValueError(f"工作表 '{sheet_name}' 未找到。可用的工作表: {', '.join(available_sheets)}")
        ...
        raise ValueError(f"无法打开Excel文件，文件路径可能不正确或文件已损坏: {file_path}")
...
if __name__ == "__main__":
    # 修复文件路径，使用原始字符串r前缀或正斜杠
    file_path = r"else\Test\excel\演员单位.xlsx"  # 使用r前缀创建原始字符串
    # 或者使用正斜杠: file_path = "else/Test/excel/演员单位.xlsx"
    ...
    try:
        ...
        with zipfile.ZipFile(file_path) as zip_file:
            ...
            for sheet_elem in workbook_root.findall('.//sheet'):
                print(f"- {sheet_elem.get('name')}")
            
            # 获取第一个工作表的名称
            first_sheet = workbook_root.find('.//sheet').get('name')
            print(f"自动选择第一个工作表: {first_sheet}")
