...
class TSGenerator:
    ...
    def generate_ts_file(self,excel_name:str,sheets_data: List[Tuple[str, Dict[str, Any]]],output_dir: str, baize_frame: bool = True):
        ...
        if len(sheets_data) == 1:
            ...
            output_file_name = self.ts_handle(
                excel_name, metadata, output_dir, baize_frame
            )
    ...
    def ts_handle(
        ...
        try:
            ...
            if baize_frame:
                # 使用白泽框架的实现
                ts_code.append("        // 使用白泽框架")
                ts_code.append("        BzTool.ExcelToData(data, {});")
            else:
                # 不使用白泽框架的实现
                ts_code.append("        // 不使用白泽框架")
                ts_code.append("        // 在这里添加自定义数据处理代码")
