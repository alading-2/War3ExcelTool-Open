...
def process_files(config: dict):
    ...
    # ini_input_path = config.get("ini_input_path", "") # 移除
    ...
    if input_path and (convert_to_ts or convert_to_lua or convert_to_ini):
        logger.info(f"Scanning for Excel files in: {input_path}")
    ...
    if input_path and convert_ini_to_excel:
        logger.info(f"Scanning for Ini files in: {input_path}")
        ini_files = find_excel_files(input_path, (".ini",)) # 使用 input_path
