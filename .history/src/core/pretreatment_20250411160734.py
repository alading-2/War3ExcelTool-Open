...
def extract_preprocessor_directives(
    ...
    """
    df: pd.DataFrame = excel_table.data
    ...
    pattern = r"(#\w+)(?:\s+([^\s#]+))?"
    # 遍历可能包含指令的行
    for row_idx, series in df_new.iloc[
        :check_rows
    ].iterrows():  # 排除最后一行，因为最后一行必须是表头
        # 遍历这一行的series
        for col_idx, value in series.items():
            # 跳过空单元格
...
PREPROCESSOR_ACTIONS: Dict[str, PreprocessorFn] = {
    ...
    lambda value, directive_value: directive_value if pd.isna(value) else value,
    "#path":
...
def apply_column_preprocessors(
    df: pd.DataFrame, preprocessors: Dict[str, Dict[str, Any]]
) -> pd.DataFrame:
    """
                ...
                if len(df_new) > 2:
                    ...
                    def process_cell(cell_value):
                        for (
                            ...
                            VALID_PREPROCESSORS
                        ):  # 遍历所有设定的预处理指令 ["#default", "#path"]
                            if prep in col_preprocessors:  # 该列包含预处理指令
                                # 使用映射调用对应的处理函数
                                if prep in PREPROCESSOR_ACTIONS:
                                    cell_value = PREPROCESSOR_ACTIONS[prep](
                                        cell_value, col_preprocessors[prep]
                                    )
                        # 如果值不是字符串也不是NaN，保持原值
                        return cell_value

                    # 使用apply方法向量化处理列
                    df_new.iloc[2:, col_idx] = df_new.iloc[2:, col_idx].apply(
                        process_cell
                    )
