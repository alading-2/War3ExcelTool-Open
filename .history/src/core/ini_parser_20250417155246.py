...
from ..utils.logger import logger # 从相对路径导入自定义的 logger (似乎未使用，可以考虑移除)
...
class IniParser:
    ...
    def parse_ini(self, path: str) -> List[Dict[str, str]]: # 更新返回类型提示
        ...
        try:
            # 尝试读取 INI 文件，使用 utf-8 编码以支持中文等非 ASCII 字符
            ...
            # 遍历 INI 文件中的所有节 (sections)
            for section_name in self.config.sections():
                # 为每个节创建一个字典，并添加 "id" 键，值为节的名称
                section_dict = {"id": section_name}
                # 遍历当前节中的所有键值对 (items)
                for key, value in self.config.items(section_name):
                    # 将键值对添加到节字典中，值保持为字符串类型
