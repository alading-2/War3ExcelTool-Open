...
class ExcelTable:
    ...
    if preprocessors:
        df = apply_column_preprocessors(df, preprocessors)
    ...
    for idx, key in enumerate(keys):
        if pd.notna(key) and str(key).strip() != "":
            comment = (
                comments[idx]
                if idx < len(comments) and pd.notna(comments[idx])
                else None
            )
    ...
    self._data.iloc[2:] = self._data.iloc[2:].apply(excel_parser._value_handle)

    # 创建column_info字典
    self.column_info = {}
    for i, key in enumerate(valid_col_keys):
        type = types[i]
        self.column_info[key] = ColumnInfo(
            key=key,
            type=type,  # 默认类型为'any'，因为类型推断不是此类的一部分
            comment=comment,
            preprocessing_rules=(preprocessors[key] if key in preprocessors else None),
        )
