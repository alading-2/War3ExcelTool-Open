...
class ExcelParser:
        ...
        try:
            ...
            for row_idx, row in enumerate(sheet.rows, 1):
                ...
                for col_idx, cell in enumerate(row, 1):
                    try:
                        # 处理单元格
                        formatted_text = self._process_cell(cell)
                        if formatted_text:  # 只有当有颜色标记时才添加到映射
                            position = f"{row_idx}_{col_idx}"
                            color_map[position] = formatted_text
                    except Exception as cell_error:
                        self.logger.warning(
                            f"处理单元格({row_idx}, {col_idx})时出错: {str(cell_error)}")
                        continue
    ...
    def _process_cell(self, cell) -> Optional[str]:
        """
        处理单个单元格，提取其文本和颜色信息
        ...
        Returns:
            Optional[str]: 带颜色标记的文本字符串，如果没有颜色则返回None
        """
        ...
        # 处理富文本单元格
        if self._has_rich_text(cell):
            formatted_text = self._process_rich_text(cell)
            return formatted_text if formatted_text != cell_text else None
            
        # 处理普通文本单元格
        color = self._get_cell_color(cell)
        if color and color.upper() != '000000':
            return f"|cff{color}{cell_text}|r"
            
        return None
    
    def _has_rich_text(self, cell) -> bool:
        """
        检查单元格是否包含富文本
        ...
        Returns:
            bool: 是否包含富文本
        """
        return (hasattr(cell, 'rich_text') and 
                cell.rich_text is not None and 
                len(cell.rich_text) > 0)
    ...
    def _get_cell_color(self, cell) -> Optional[str]:
        ...
        Returns:
            Optional[str]: 颜色代码，如果没有颜色则返回None
        """
        try:
            if (cell.font and 
                hasattr(cell.font, 'color') and 
                cell.font.color and 
    ...
    def _process_rich_text(self, cell) -> str:
        ...
        try:
            ...
            for fragment in cell.rich_text:
                ...
                text = fragment.text
                color = self._get_fragment_color(fragment)
                ...
                if color == current_color:
                    ...
                    # 处理之前累积的文本
                    if current_text:
                        result.append(self._format_text_with_color(current_text, current_color))
            ...
            return str(cell.value)  # 出错时返回原始文本
    
    def _get_fragment_color(self, fragment) -> Optional[str]:
        """
        获取富文本片段的颜色代码
        ...
        """
        try:
            if (hasattr(fragment, 'font') and 
                fragment.font and 
                hasattr(fragment.font, 'color') and 
                fragment.font.color and 
                hasattr(fragment.font.color, 'rgb') and 
                fragment.font.color.rgb):
                
                color = fragment.font.color.rgb
                return color[2:] if color.startswith('FF') else color
        except AttributeError:
            pass
        return None
    
    def _format_text_with_color(self, text: str, color: Optional[str]) -> str:
        """
        使用颜色代码格式化文本
