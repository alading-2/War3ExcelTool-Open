...
class ExcelParser:
    ...
    def parse_excel(self, file_path: str) -> List[Tuple[str, Dict[str, Any]]]:
        ...
        try:
            ...
            for sheet_name in sheet_names:
                ...
                # 检查DataFrame是否有足够的行
                if df.shape[0] < 3:
                    raise ValueError("输入的DataFrame必须至少有3行")
                ...
                commoms = excel_table.get_comments()
                ...
                # 数据处理
                df.iloc[2:] = df.iloc[2:].apply(self.value_handle_single)
    ...
    def infer_type_single(
        ...
        self.logger.debug(
            f"Value '{value}' of type {type(value)} cannot be directly mapped, inferring as 'any'."
        )
        return "any"
    ...
    def value_handle_single(self, value: Any) -> Any:
        """
