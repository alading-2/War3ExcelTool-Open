...
class LuaGenerator:
    ...
    def generate_lua_file(self, excel_name: str, sheets_data: List[Dict[str, Any]], output_dir: str):
        """
        生成Lua文件
        ...
        if len(non_object_sheets) == 1:
            ...
            # 生成Lua文件
            output_file_name = self.lua_handle(excel_name, metadata, output_dir)
    ...
    def lua_handle(self, excel_name: str, metadata: Dict[str, Any], output_dir: str):
        """
        生成Lua文件内容
        ...
        try:
            ...
            lua_code.extend(
                [
                    f"{keys_name} = {{",  # 表开始
                    "    " + ", ".join(f'"{key}"' for key in keys if key != "code"),  # 所有键名
                    "}",
                    "",  # 空行分隔
                ]
            )
            ...
            for row_idx in range(2, excel_table.num_rows):
                ...
                for col_idx, key in enumerate(keys):
                    ...
                    lua_value_str = self._value_to_lua_string(value, indent_level=2)
                    lua_code.append(f'        ["{key}"] = {lua_value_str},')  # 键总是字符串形式
            ...
            # 构造输出文件路径
            output_file = os.path.join(output_dir, f"{extension}_{excel_name}.lua")
