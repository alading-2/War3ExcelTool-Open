...
class LuaGenerator:
    ...
    def _value_to_lua_string(self, value: Any, indent_level: int = 2) -> str:
        ...
        if isinstance(value, dict):
            ...
            # 对于无法直接转换的类型，尝试用repr，并给出警告
            self.logger.warning(f"无法直接转换为Lua的类型: {type(value)}，尝试使用repr()。")
            # 仍然需要转义repr可能产生的字符串中的特殊字符
            repr_str = repr(value)
            escaped_value = repr_str.replace('\\', '\\\\').replace(
                '"', '\\"').replace('\n', '\\n').replace('\r', '\\r')
            return f'"{escaped_value}"'  # 将未知类型也视为字符串
    ...
    def lua_handle(self, excel_name: str, metadata: Dict[str, Any],
        ...
        try:
            ...
            self.logger.error(f"异常详情: ", exc_info=True)  # 添加详细堆栈信息
            raise
