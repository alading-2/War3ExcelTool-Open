    ...
    def write_to_excel(self, data_dict: Dict[str, Dict], output_path: str) -> bool:
        ...
        try:
            ...
            with pd.ExcelWriter(output_path, engine='openpyxl', mode='a' if os.path.exists(output_path) else 'w') as writer:
                for type, type_data in data_dict.items():
                    if type not in self.sheet_mapping:
                        continue
                        
                    # 转换 ConfigParser 到 DataFrame
                    data = []
                    for id,ini_data in type_data:
                        row = {'ID': id}
                        row.update(dict(type_data.items(ini_data)))
                        data.append(row)
                    
                    df = pd.DataFrame(data)
                    sheet_name = self.sheet_mapping[type]
