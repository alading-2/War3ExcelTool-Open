...
class War3IniParser(configparser.ConfigParser):
    ...
    def read_file(self, f, source=None):
        ...
        def save_to_ini(output_path: str) -> None:
            ...
            try:
                # 确保输出路径包含.ini扩展名
                if not output_path.lower().endswith('.ini'):
                    output_path += '.ini'
                
                # 转换为绝对路径
                if not os.path.isabs(output_path):
                    output_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), output_path)
                ...
                logger.info(f"成功保存INI文件到: {output_path}")
            except Exception as e:
                logger.error(f"保存INI文件到 {output_path} 时出错: {e}", exc_info=True)
                raise
        save_to_ini("else/Test/test.ini")
        # 将处理后的内容传递给标准解析器
        import io
...
class IniParser:
    ...
    def __init__(self):
        ...
        """
        self.config = War3IniParser(strict=False) # 允许重复键
        self.logger = logger
