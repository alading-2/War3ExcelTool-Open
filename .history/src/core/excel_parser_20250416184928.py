...
class ExcelParser:
    ...
    def parse_excel(self, file_path: str) -> List[Tuple[str, Dict[str, Any]]]:
        ...
        try:
            ...
            for sheet_name in sheet_names:
                ...
                # 检查DataFrame是否有足够的行
                if df.shape[0] < 3:
                    raise ValueError(f"{file_path}的表“{sheet_name}”行数<3，不解析")

                # 删除键名为空或=""的列
                # df.loc[]会去掉False的列
                condition = df.iloc[1].map(
                    lambda x: excel_table.custom_isna(x) or x == r'""'
                )
                df = df.loc[:, ~condition]
                ...
                # # 解析实际数据行（第三行及以下）
                # # 注意：_parse_data_rows 现在可能不再需要，数据已在excel_table中
                # data_rows = self._parse_data_rows(df, metadata) # 旧代码，可能已废弃
    ...
    def infer_type_single(
        ...
        """
        if Excel_Table.custom_isna(value):  # 首先处理Pandas的NA值
            return None
        ...
        if isinstance(value, str):  # 字符串需要进一步判断
            ...
            if re.fullmatch(
                ...
                # 正则匹配成功，认为是数字类型
                # float(value_stripped.replace(",", "")) # 实际转换在value_handle中进行
                return "number"
        ...
        self.logger.debug(
            f"值 '{value}' 的类型 {type(value)} 无法直接映射，推断为 'any'。"
        )
        return "any"
    ...
    def infer_types(self, df: pd.DataFrame) -> Dict[str, str]:
        ...
        for j in range(num_columns):
            ...
            key = df.iloc[1, j] # 使用第二行的值作为键

            column_types = set()  # 存储该列遇到的所有非空值的推断类型
            has_non_na = False  # 标记该列是否有非空（非NA）值
            for value in values:
                if Excel_Table.custom_notna(value):  # 只处理非空值
                    has_non_na = True
                    # 推断单个值的类型
                    ...
                    ):  # 理论上 custom_notna(value) 为真时不会是 None
            ...
            if not has_non_na or not column_types:
                ...
                column_final_type = " | ".join(sorted(list(column_types))) # 生成联合类型
    ...
    def value_handle(self, value: Any) -> Any:
        ...
        inferred_type = self.infer_type_single(value) # 推断类型
        # print("value:", value, "type:", inferred_type)
        # 处理数字类型值
        if inferred_type == "number":
            # 移除数字字符串中的逗号 (例如 '1,000' -> '1000')
            return str(value).replace(",", "")

        if inferred_type == "boolean":
            return str(value).lower() # 转换为小写 'true' 或 'false'

        # 处理字符串类型值
        if isinstance(value, str) and inferred_type == "string":
            # 检查字符串是否代表数字（使用正则表达式匹配）
            # ^表示开始，-?表示可选的负号，\d+表示一个或多个数字
            # (\.\d+)?表示可选的小数部分，$表示结束
            if re.match(r"^-?\d{1,3}(?:,\d{3})*(?:\.\d+)?([eE][+-]?\d+)?$", value):
                return str(value).replace(",", "") # 如果是数字字符串，移除逗号

            # 检查字符串是否代表布尔值（"true"或"false"）
            if value.lower() in ("true", "false"):
                return value.lower() # 如果是布尔字符串，转小写

            # 将任意个\替换为\\ (处理转义)
            value = re.sub(r"\\+", r"\\", value)

            # 先去掉首尾空白，再去掉可能存在的多余的双引号
            if value != "":
