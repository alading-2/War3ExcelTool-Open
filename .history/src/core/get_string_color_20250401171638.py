    ...
    try:
        with zipfile.ZipFile(file_path) as zip_file:
            ...
            if not sheet_elements:
                ...
                sheet_elements = [elem for elem in sheet_elements if elem.tag.endswith('sheet')]
            ...
            if not cell_elem:
                for elem in sheet_root.findall('.//*'):
                    if elem.tag.endswith('c') and elem.get('r') == cell_ref:
                        cell_elem = elem
                        print(f"DEBUG: 使用通用查找找到单元格: {cell_ref}")
                        break
            ...
            if cell_type == 's':  # 共享字符串
                ...
                v_elem = cell_elem.find('x:v', namespaces) or cell_elem.find('.//*[local-name()="v"]')
            ...
            if not si_elements:
                si_elements = []
                for elem in shared_strings_root.findall('.//*'):
                    if elem.tag.endswith('si'):
                        si_elements.append(elem)
                print(f"DEBUG: 使用标签名称查找找到 {len(si_elements)} 个共享字符串项")
            ...
            if not r_elements:
                r_elements = []
                for elem in si_elem.findall('.//*'):
                    if elem.tag.endswith('r'):
                        r_elements.append(elem)
                print(f"DEBUG: 使用标签名称查找找到 {len(r_elements)} 个富文本元素")
                
            if r_elements:  # 富文本
                print(f"DEBUG: 发现富文本，共有 {len(r_elements)} 个部分")
                for r in r_elements:
                    t_elements = r.findall('.//x:t', namespaces)
                    if not t_elements:
                        t_elements = []
                    ...
                    rpr_elements = r.findall('.//x:rPr', namespaces)
                    if not rpr_elements:
                        rpr_elements = []
                        for elem in r.findall('.//*'):
                            if elem.tag.endswith('rPr'):
                                rpr_elements.append(elem)
                    
                    rpr = rpr_elements[0] if rpr_elements else None
                    color = None
                    
                    if rpr is not None:
                        color_elements = rpr.findall('.//x:color', namespaces)
                        if not color_elements:
                            color_elements = []
                            for elem in rpr.findall('.//*'):
                                if elem.tag.endswith('color'):
                                    color_elements.append(elem)
                        
                        color_elem = color_elements[0] if color_elements else None
