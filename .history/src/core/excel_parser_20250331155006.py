...
class ExcelParser:
    ...
    def _extract_colors(self,
        ...
        Returns:
            颜色映射字典: {单元格位置: 带颜色标记的字符串}，位置格式为"行号_列号"
        """
        ...
        # 获取指定的工作表或活动工作表
        sheet = wb[
            sheet_name] if sheet_name and sheet_name in wb.sheetnames else wb.active

        # 创建空字典，用于存储单元格位置到带颜色标记字符串的映射
        color_map = {}

        # 遍历工作表中的行（从第三行（索引2）开始，即实际数据行，第1列（索引0）开始）
        for row_idx, row in enumerate(sheet.iter_rows(min_row=2), 2):
            for col_idx, cell in enumerate(row):
                ...
                # 检查单元格的值是否是字符串类型，并且单元格有字体属性
                if isinstance(cell.value, str) and cell.font:
                    text = cell.value
                    ...
                    for char_idx, char in enumerate(text):
                        ...
                        if font_color and font_color.rgb:
                            # 转换为TypeScript颜色格式
                            color_code = f"|cff{font_color.rgb[2:]}"  # 去除alpha通道前缀
                            # 如果当前字符的颜色与前一个字符相同，则继续累积
                            if color_code == current_color_code:
                                current_color_text += char
                            else:
                                # 如果有累积的文本，则将其添加到colored_text中，并重置颜色代码和文本
                                if current_color_text:
                                    colored_text += f"{current_color_code}{current_color_text}|r"
                                current_color_code = color_code
                                current_color_text = char
                        else:
                            # 如果当前字符没有颜色，则将其添加到colored_text中，并重置颜色代码
                            if current_color_text:
                                colored_text += f"{current_color_code}{current_color_text}|r"
                                current_color_code = None
                            colored_text += char
                ...
                # 将带颜色标记的字符串存储到颜色映射字典中
                if colored_text:
                    color_map[position] = colored_text
