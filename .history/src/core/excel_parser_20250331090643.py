...
class ExcelParser:
    ...
    def parse_excel(self, file_path: str, sheet_name: Optional[str] = None) -> Union[Tuple[Dict[str, Any], List[str], List[Dict], Dict[str, str]], List[Tuple[str, Dict[str, Any], List[str], List[Dict], Dict[str, str]]]]:
        """
        解析Excel文件，提取数据和结构信息
        ...
        try:
            ...
            # 如果指定了sheet_name且该sheet存在，解析指定sheet
            if sheet_name and sheet_name in sheet_names:
                self.logger.info(f"解析工作表: {sheet_name}")
                metadata, keys, data_rows, color_map = self._parse_single_sheet(file_path, sheet_name)
                return metadata, keys, data_rows, color_map
            
            # 如果没有指定sheet_name或指定的sheet不存在，解析所有sheet
            results = []
            
            # 遍历所有sheet并解析
            for sheet in sheet_names:
                self.logger.info(f"解析工作表: {sheet}")
                metadata, keys, data_rows, color_map = self._parse_single_sheet(file_path, sheet)
                results.append((sheet, metadata, keys, data_rows, color_map))
    ...
    def _parse_single_sheet(self, file_path: str, sheet_name: str) -> Tuple[Dict[str, Any], List[str], List[Dict], Dict[str, str]]:
        """
        解析单个Excel工作表，提取其数据和结构信息
