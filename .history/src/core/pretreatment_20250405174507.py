...
def extract_preprocessor_directives(
    ...
    check_rows = min(len(df_copy), max_directive_rows)
    if check_rows < 3:
        # 至少需要两行（可能包含指令的行和表头行）
        logger.warning("DataFrame行数不足，无法提取预处理指令")
        return {}, df_copy  # 返回空结果和原始DataFrame
    ...
    for row_idx in range(check_rows - 1):  # 排除最后一行，因为最后一行必须是表头
        # 遍历这一行的所有单元格
        for col_idx, cell_value in enumerate(df_copy.iloc[row_idx]):
            # 跳过空单元格
            ...
            if directives:
                ...
                # 安全地修改DataFrame
                df_copy.iloc[row_idx, col_idx] = cleaned_cell
                
                # 获取列键名，并去除首尾空格
                col_key = str(df_copy.iloc[1, col_idx]).strip()
    ...
    return column_preprocessors, df_copy
...
def apply_column_preprocessors(
    ...
    try:
        # 获取所有列键名
        raw_keys = df_clean.iloc[1].tolist()
        ...
        for col_idx, key_any in enumerate(raw_keys):
            ...
            if pd.notna(key_any) and key != "" and key in preprocessors:
                ...
                if len(df_clean) > 2:
                    ...
                    # 使用apply方法向量化处理列
                    df_clean.iloc[2:, col_idx] = df_clean.iloc[2:, col_idx].apply(process_cell)
