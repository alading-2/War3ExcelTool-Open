...
class ExcelParser:
    ...
    def parse_excel(self, file_path: str) -> Tuple[Dict[str, Any], List[str], List[Dict], Dict[str, str]]:
        ...
        if not os.path.exists(file_path):
            self.logger.error(f"Excel文件不存在: {file_path}")  # 记录错误日志
            raise FileNotFoundError(f"Excel文件不存在: {file_path}")  # 抛出文件不存在异常
        
        try:
            # 使用pandas读取Excel文件内容，header=None表示不将第一行作为列名
            df = pd.read_excel(file_path, header=None)
            
            # 读取Excel文件中的单元格颜色信息
            color_map = self._extract_colors(file_path)  # 下划线开头的方法表示私有方法，只在类内部使用
            
            # 提取第一行作为注释信息（列说明）
            comments = df.iloc[0].tolist()  # iloc是pandas的索引方法，[0]表示第一行，tolist()将行转换为列表
            ...
            # 创建元数据字典，包含注释、键名和文件名
            metadata = {
                "comments": comments,  # 注释信息（第一行）
                "keys": keys,          # 键名列表（第二行）
                "filename": os.path.basename(file_path)  # 提取文件名，不包含路径
            }
            
            # 返回解析结果：元数据、键名列表（不包括ID列）、数据行列表、颜色映射
    ...
    def parse_excel_sheets(self, file_path: str) -> List[Tuple[str, Dict[str, Any], List[str], List[Dict], Dict[str, str]]]:
        """
        解析Excel文件的所有工作表(sheets)，提取数据和结构信息
        
        这个方法会遍历Excel文件中的所有工作表，并对每个工作表进行解析。
        
        Args:
            file_path: Excel文件路径，表示要解析的Excel文件的位置
            
        Returns:
            一个列表，每个元素包含五个元素的元组(Tuple)：
            - sheet名称: 字符串，工作表的名称
            - 元数据: 字典，包含注释、键名列表和文件名等信息
            - 键列表: 列表，数据字段的键名列表（不包括ID列）
            - 数据列表: 列表，每个元素是一个字典，表示一行数据
        ...
        try:
            ...
            # 如果只有一个sheet，直接使用原来的方法解析
            if len(sheet_names) == 1:
                metadata, keys, data_rows, color_map = self.parse_excel(file_path)
                return [(sheet_names[0], metadata, keys, data_rows, color_map)]
            ...
            for sheet_name in sheet_names:
                ...
                # 将当前sheet的解析结果添加到列表
                results.append((sheet_name, metadata, keys[1:], data_rows, color_map))
