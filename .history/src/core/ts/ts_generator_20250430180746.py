...
class TSGenerator:
    ...
    def generate_ts_file(self, excel_name: str, sheets_data: List[Dict[str, Any]], output_dir: str):
        # 过滤掉物编表格
        non_object_sheets = [metadata for metadata in sheets_data if not metadata.get("is_object_table", False)]

        if not non_object_sheets:
            self.logger.info(f"所有sheet都是物编表格，跳过生成TS文件: {excel_name}")
            return
        ...
        if len(non_object_sheets) == 1:
            ...
            # 生成TS文件
            output_file_name = self.ts_handle(excel_name, metadata, output_dir)
            self.logger.info(f"成功生成 ts 文件: {output_file_name}")
            ...
            for metadata in non_object_sheets:
                ...
                try:
                    # 生成TS文件
                    output_file_name = self.ts_handle(
                        sheet_excel_name,
                        metadata,
                        output_dir,
                    )

                    self.logger.info(f"正在处理{sheet_name}，成功生成: {output_file_name}")
                except Exception as sheet_e:
                    self.logger.error(f"处理sheet {sheet_name} 时出错: {str(sheet_e)}")
    ...
    def ts_handle(
        ...
        try:
            ...
            # 接口键名常量
            interface_keys_name = f"{extension}_inte_keys_{excel_name}"
            # 接口名称
            interface_name = f"{extension}_inte_{excel_name}"
            # 数据数组名称
            data_name = f"{extension}_data_{excel_name}"
            # 类名
            class_name = f"{extension}_{excel_name}"
            ...
            for col_idx, key in enumerate(keys):
                ...
                # 添加到接口属性定义列表（字段名和类型）
                interface_props.append(f'        "{key}"?: {types[key]};    // {comment}')
            ...
            for row_idx in range(2, excel_table.num_rows):
                ...
                for col_idx, key in enumerate(keys):
                    ...
                    # 字符串类型需要用引号包裹
                    if type == "string":
                        # 使用 repr(value)[1:-1] 来获取字符串的原始表示形式，并去掉两端的引号。
                        row_entries.append(f'        "{key}": "{repr(value)[1:-1]}"')
                    else:
                        # 其他类型直接使用值
                ...
                if row_entries:
                    ...
                    data_key = excel_table.get_value(row_idx, 0)  # 获取当前行第一列作为数据项的key值
                    # 去除data_key中的颜色码
                    if isinstance(data_key, str):
                        # 使用正则表达式移除|cXXXXXXXX颜色码和|r结束标记
                        data_key = re.sub(r"\|c[0-9a-fA-F]{8}|\|r", "", data_key)
                    data_entries.append(f'    "{data_key}":{{\n{",\n".join(row_entries)}\n    }},')
            ...
            # 完成类定义
            ts_code.extend(["    }", "}", ""])  # Start方法结束  # 类定义结束  # 文件末尾空行

            # 确保输出目录存在，如果不存在则创建，前面已经创建过了
            # os.makedirs(output_dir, exist_ok=True)

            # 构造输出文件路径：输出目录 + 文件名
            output_file = os.path.join(output_dir, f"{extension}_{excel_name}.ts")
            # 打开文件并写入内容
    ...
    def extract_ts_code(self, excel_table: ExcelTable, keys: List[str]) -> Tuple[List[str], List[str]]:
        """
        从数据中提取TypeScript代码
        ...
        for code in code_list:
            # 不为空
            if code and ExcelTable.custom_notna(code):
                ...
                other_lines = [line for line in code.split("\n") if not line.strip().startswith("import ")]
                # 将提取的其他代码添加到其他代码列表中
