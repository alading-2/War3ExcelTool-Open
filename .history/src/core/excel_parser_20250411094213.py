    ...
    def parse_excel(self, file_path: str) -> List[Tuple[str, Dict[str, Any]]]:
        ...
        try:
            ...
            for sheet_name in sheet_names:
                ...
                df = excel_table._data
                # 检查DataFrame是否有足够的行
                if df.shape[0] < 2:
                    raise ValueError("输入的DataFrame必须至少有2行")

                # 删除键名为空的行，//开头或空单元格开头的行
                ...
                self._data.iloc[2:] = self._data.iloc[2:].apply(
                    excel_parser._value_handle
                )
                ...
                for i, key in enumerate(valid_col_keys):
                    ...
                    self.column_info[key] = ColumnInfo(
                        ...
                        comment=comment,
                        preprocessing_rules=(
                            preprocessors[key] if key in preprocessors else None
                        ),
                    )
