...
class War3IniParser:
    ...
    def read_file(self, path, source=None):
        ...
        # 1. 将 -- 注释转换为 # 注释
        # \1：这是正则表达式的反向引用，表示捕获组匹配的内容
        # (.*?) 是第一个捕获组（非贪婪模式），匹配 -- 后的注释内容
        ...
        multiline_key = None   # 当前处理的多行字符串的键名
        multiline_value = []   # 当前处理的多行字符串的值
        ...
        for line in lines:
            ...
            if line.startswith('[') and line.endswith(']'):
                ...
                if in_multiline:
                    if multiline_start == '{':
                        # 数组格式 HeroDur = {7.0, 7.0, 7.0, 7.0} 保持原样
                        value = ' '.join(multiline_value)
                    else:  # [=[...]=]
                        # 多行字符串格式，保留原始格式包括[=[和]=]
                        value = '\n'.join(multiline_value)
                    buffer.append((multiline_key, value))
                    in_multiline = False
            ...
            if not in_multiline:
                ...
                if multiline_start == '{' and line.endswith('}'):
                    value = ' '.join([l.strip().rstrip(',') for l in multiline_value])
                    buffer.append((multiline_key, value))
                    in_multiline = False
                # 处理多行字符串的结束
                elif multiline_start == '[=[' and line.endswith(']=]'):
                    value = '\n'.join(multiline_value[1:-1])  # 去掉[=[ 和 ]=]
                    buffer.append((multiline_key, value))
                    in_multiline = False
