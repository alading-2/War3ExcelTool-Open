...
from src.core.pretreatment import extract_preprocessor_directives, apply_preprocessors, PREPROCESSOR_ACTIONS
...
class ExcelParser:
    ...
    def parse_excel(
        ...
        try:
            ...
            for sheet_name in sheet_names:
                ...
                # 从前几行提取预处理指令
                column_preprocessors, directive_rows, header_row_idx = extract_preprocessor_directives(df_raw)
                
                # 根据识别的表头行重新读取DataFrame
                df = pd.read_excel(file_path,
                                  sheet_name=sheet_name,
                                  header=None,
                                  skiprows=list(directive_rows))  # 跳过指令行
                ...
                for i, key_any in enumerate(raw_keys):
                    ...
                    if pd.notna(key_any) and key != "":
                        ...
                        preproc_rules = {}
                        col_tuple = (i, key)
                        if col_tuple in column_preprocessors:
                            preproc_rules = column_preprocessors[col_tuple]
                ...
                metadata = {
                    ...
                    "columns_info": columns_info,                  # 包含所有ColumnInfo对象的列表
                }
    ...
    def _parse_data_rows(self, df: pd.DataFrame,
        ...
        for idx, row in df.iloc[2:].iterrows():
            ...
            if first_cell_value is None or pd.isna(first_cell_value) or first_cell_value == "//":
                continue # 跳到下一行
            ...
            for i, value in enumerate(row):
                ...
                if col_info.preprocessing_rules:
                    # 应用预处理规则
                    value = apply_preprocessors(value, col_info.preprocessing_rules)
