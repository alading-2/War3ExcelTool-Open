...
class ExcelParser:
    ...
    def parse_excel(
        ...
        try:
            ...
            for sheet_name in sheet_names:
                ...
                # 提取第一行作为注释信息（列说明），首尾去除空格
                comments = [str(val).strip() for val in df.iloc[0].tolist()]

                # 提取第二行作为数据的键名，首尾去除空格
                keys = [str(val).strip() for val in df.iloc[1].tolist()]

                # 提取第三行数据推断类型
                types = [self.infer_type(val) for val in df.iloc[2].tolist()]

                # 解析实际数据行（第三行及以下）
                data_rows = self._parse_data_rows(df, keys, types)
                ...
                # 将当前sheet的解析结果添加到列表
                results.append(
                    (sheet_name, metadata, keys, data_rows, color_map))
    ...
    def _parse_data_rows(self, df: pd.DataFrame, keys: List,
    ...
    def infer_type(self, value: Any) -> str:
        ...
        # 如果值是布尔类型，返回"boolean"
        if isinstance(f"{value}", bool):  # isinstance检查值是否为指定类型的实例
            return "boolean"

        # 如果值是整数或浮点数，返回"number"
        if isinstance(f"{value}", (int, float)):  # 可以同时检查多种类型
            return "number"
