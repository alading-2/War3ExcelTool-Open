    ...
    def generate_ts_file(self,
        ...
        try:
            ...
            for col_idx, key in enumerate(keys):
                ...
                # 添加到接口属性定义列表（字段名和类型）
                interface_props.append(f'    "{key}"?: {types[col_idx]};')
            ...
            for row_idx in range(2, excel_table.num_rows):
                ...
                # 遍历所有键，处理当前行中的每个字段
                for col_idx, key in enumerate(keys):
                    value = excel_table.get_value(row_idx,
                                                  col_idx)  # 获取当前行的单元格值
                    # value不为空，也不是code列
                    try:
                        if pd.isna(value) or key == "code":
                            continue
                    except ValueError:  # 处理value是数组的情况
                        if pd.isna(value).any() or key == "code":
                            continue
