    ...
    def write_to_excel(self,
        ...
        try:
            ...
            if os.path.exists(self.template_path):
                excel_basename = os.path.basename(output_path)
                if excel_basename != "物编.xlsx":
                    # 如果目标文件名不是"物编.xlsx"，则修改复制后的文件名
                    temp_output_path = os.path.join(
                        os.path.dirname(output_path), "物编.xlsx")
                    shutil.copyfile(self.template_path, temp_output_path)
                    os.rename(temp_output_path, output_path)
                else:
                    # 如果目标文件名就是"物编.xlsx"，直接复制
