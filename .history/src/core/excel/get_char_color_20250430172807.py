...
class ExcelCharColorReader:
    ...
    def get_cell_with_color_tags(self, sheet_name: str, cell_address: str) -> Tuple[str, bool]:
        ...
        try:
            ...
            # 获取单元格中每个字符的格式信息
            # xlwings的characters索引从1开始，且是闭区间[start, end]
            for i in range(1, len(cell_value) + 1):
                # 使用xlwings获取字符的颜色信息
                try:
                    # 使用正确的索引方式：从1开始，每次获取一个字符
                    char_range = cell.characters[i:i]
                    ...
                    try:
                        ...
                        self.logger.debug(f"获取第{i}个字符颜色失败: {str(e)}")
                        rgb_color = (0, 0, 0)
                except Exception as e:
                    self.logger.debug(f"获取第{i}个字符范围失败: {str(e)}")
                    rgb_color = (0, 0, 0)
