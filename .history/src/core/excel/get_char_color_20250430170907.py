...
class ExcelCharColorReader:
    ...
    def get_cell_with_color_tags(self, sheet_name: str, cell_address: str) -> Tuple[str, bool]:
        ...
        try:
            ...
            # 检查是否可以安全地获取颜色信息
            try:
                # 尝试获取第一个字符的颜色，测试颜色访问是否正常工作
                test_char = cell.characters[1:2]
                test_font = test_char.font
                test_color = test_font.color
                can_get_color = True
                self.logger.debug("颜色访问测试成功，将获取每个字符的颜色")
            except Exception as e:
                # 如果测试失败，标记无法获取颜色
                can_get_color = False
                self.logger.warning(f"颜色访问测试失败，将直接返回原始文本: {str(e)}")
                return cell_value, False

            # 如果不能获取颜色信息，直接返回原始文本
            if not can_get_color:
                return cell_value, False
            ...
            for i in range(1, len(cell_value) + 1):
                try:
                    ...
                    # 如果处理某个字符出错，就把这个字符当作黑色处理
                    self.logger.debug(f"处理第{i}个字符时出错: {str(e)}，当作黑色处理")
                    if current_color == (0, 0, 0):
                        # 如果当前是黑色，直接添加
                        current_text += cell_value[i - 1]
                    else:
                        # 如果当前不是黑色，结束当前文本段并开始新文本段
                        if current_text:
                            if current_color and current_color != (0, 0, 0):
                                result += f"{self.rgb_to_formatted_color(current_color)}{current_text}|r"
                            else:
