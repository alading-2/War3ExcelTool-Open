        ...
        if isinstance(value, str) and type in ("any"):
            ...
            if (cleaned_value.startswith("{") and cleaned_value.endswith("}")) or \
               (cleaned_value.startswith("[") and cleaned_value.endswith("]")):
                try:
                    # 步骤 1：尝试处理键的引号问题（给无引号的键添加双引号）
                    # 这可能不完美，但保留原始意图
                    value1 = re.sub(r"(\b\w+\b)(?=\s*:)", r'"\1"', cleaned_value) # 更精确匹配键

                    # 步骤 2：尝试处理末尾的逗号问题
                    value2 = re.sub(r",\s*([}\]])", r"\1", value1)

                    # 步骤 3：使用 json.loads 解析
                    parsed_value = json.loads(value2)
                    self.logger.debug(f"成功将值 '{value}' 解析为 JSON 对象/数组。")
                    return parsed_value
                except json.JSONDecodeError as e:
                    self.logger.warning(f"无法将值 '{value}' 解析为 JSON 对象/数组: {e}。尝试 literal_eval 作为备选。")
                    # 如果 JSON 解析失败，作为备选尝试 literal_eval （注意安全风险）
                    # 在尝试 literal_eval 前，也需要处理键引号和尾随逗号
                    try:
                        # 重新应用原始的正则替换，因为 literal_eval 需要不同的格式
                        # 步骤 1 (literal_eval): 处理键的引号问题（给键添加双引号，原始逻辑）
                        value1_le = re.sub(r"(\w+):", r'"\1":', cleaned_value)
                        # 步骤 2 (literal_eval): 处理末尾的逗号问题（原始逻辑）
                        value2_le = re.sub(r",\s*}", "}", value1_le) # 注意这个正则只处理对象末尾逗号
                        value2_le = re.sub(r",\s*]", "]", value2_le) # 补充处理数组末尾逗号
                        parsed_value_le = literal_eval(value2_le)
                        self.logger.debug(f"成功将值 '{value}' 通过 literal_eval 解析。")
                        return parsed_value_le
                    except (ValueError, SyntaxError, TypeError) as le_e:
                        self.logger.error(f"尝试 JSON 和 literal_eval 解析值 '{value}' 均失败: JSONDecodeError={e}, LiteralEvalError={le_e}。返回原始字符串。")
                        return value # 如果两种解析都失败，返回原始字符串
                except Exception as ex: # 捕获其他潜在错误
                    self.logger.error(f"解析值 '{value}' 时发生意外错误: {ex}。返回原始字符串。")
                    return value
