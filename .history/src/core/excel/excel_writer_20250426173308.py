        ...
        try:
            ...
            with pd.ExcelWriter(output_path,
                ...
                for type_name, type_data in data_dict.items():
                    ...
                    dict_type_map = dict_editor_info_map[type_name]
                    # 准备数据行
                    rows_data = []
                    # 注释行
                    list_comment = []
                    for key in all_keys:
                        if key in dict_type_map:
                            # 有直接的中文注释或者匹配
                            list_comment.append(dict_type_map[key])
                        else:
                            is_part = False
                            for comment_key in dict_type_map.keys():
                                # 部分匹配
                                if comment_key in key:
                                    list_comment.append(
                                        dict_type_map[comment_key])
                                    is_part = True
                                    break
                            # 不匹配：直接添加
                            if not is_part:
