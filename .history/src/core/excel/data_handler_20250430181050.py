    ...
    def get_data_dict_json(self) -> Dict[str, Any]:
        ...
        for row_idx in range(2, self.num_rows):
            ...
            if isinstance(name_key, str):
                ...
                name_key = re.sub(r"\|c[0-9a-fA-F]{8}|\|r", "", name_key)
            if not name_key:
                continue
                ...
                if self.custom_notna(value) and key != "code":
                    ...
                    if value_type == "boolean":
                        ...
                        if isinstance(value, bool):
                            result_dict[name_key][key] = value  # Python布尔值会自动转换为JSON的true/false
                        else:
                            # 字符串情况 "true"/"false"
                            result_dict[name_key][key] = value.lower() == "true"
                        ...
                        if isinstance(value, (int, float)):
                            ...
                            try:
                                ...
                                # 将字符串形式的数字转换为实际的数字类型（包括科学计数法）
                                if "." in cleaned_value:
                                    result_dict[name_key][key] = float(cleaned_value)
                                else:
                                    result_dict[name_key][key] = int(float(cleaned_value))
                            except (ValueError, TypeError):
                                self.logger.warning("get_data_dict_json字符串转整数失败")
                                # 转换失败，保留原值
                                result_dict[name_key][key] = value
                    else:
                        # 其他类型保持不变
