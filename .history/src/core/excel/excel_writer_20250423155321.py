        ...
        try:
            ...
            with pd.ExcelWriter(output_path,
                ...
                for type_name, type_data in data_dict.items():
                    ...
                    dict_comment = War3IniParser.parse_ini(path)
                    dict_comment = dict_comment.get("editor_info_map")

                    # 注释行
                    list_dict = []
                    all_keys_lower = [key.lower() for key in all_keys]  # 小写
                    for key in all_keys_lower:
                        if key in dict_comment:
                            # 有直接的中文注释或者匹配
                            list_dict.append(dict_comment[key])
                        else:
                            is_part = False
                            for key1 in dict_comment.keys():
                                # 部分匹配
                                if key1 in key:
                                    list_dict.append(dict_comment[key1])
                                    is_part = True
                                    break
                            # 不匹配：直接添加
                            if not is_part:
                    ...
                    rows_data.append(list_dict)
                    # key值
                    rows_data.append(all_keys)
                    # 构建每一行的数据
                    for section_id, section_data in type_data.items():
                        ...
                        for key1 in all_keys[1:]:
                            value = section_data.get(key1, np.nan)
                            row[key1] = value
                        rows_data.append(row)

                    # 创建DataFrame并设置列名
                    df = pd.DataFrame(rows_data, columns=all_keys)
