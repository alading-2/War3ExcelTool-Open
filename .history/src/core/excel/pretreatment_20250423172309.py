...
def extract_preprocessor_directives(
    df: pd.DataFrame, ) -> Tuple[Dict[str, Dict[str, Any]], pd.DataFrame]:
    """
...
PREPROCESSOR_ACTIONS: Dict[str, PreprocessorFn] = {
    ...
    (directive_value if ExcelTable.custom_isna(value) else value),
    "#path":
    # #path处理：如果值不为空且为字符串类型，则将反斜杠 \ 替换为双反斜杠 \\。
    lambda value, _: (
        # 将任意个\替换为\\
        re.sub(r"\\+", r"\\", value) if ExcelTable.custom_notna(value) and
