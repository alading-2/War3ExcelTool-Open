...
class TSGenerator:
    ...
    def generate_ts_file(self,excel_name:str,sheets_data: List[Tuple[str, Dict[str, Any]]],output_dir: str):
        ...
        if len(sheets_data) == 1:
            ...
            output_file_name = self.ts_handle(
                excel_name, metadata, output_dir
            )
                ...
                for col_idx, key in enumerate(keys):
                    ...
                    if type == "string":
                        ...
                        if isinstance(value, str):
                            # 处理数组格式如 HeroDur = {7.0, 7.0, 7.0, 7.0}
                            if value.startswith('{') and value.endswith('}'):
                                row_entries.append(f'        "{key}": {value}')
                            # 处理多行字符串，转换为TypeScript格式
                            elif '\n' in value:
