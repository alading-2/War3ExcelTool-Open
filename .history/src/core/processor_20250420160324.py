...
def process_files(config: dict):
    ...
    # 获取白泽框架设置
    baize_frame = config.get("baize_frame", True)
    ...
    if parse_w3x and w3x2lni_path:
        logger.info(f"开始在 {input_path} 中搜索w3x文件...")
        try:
            from .w3x_parser import W3xParser
            
            # 搜索输入路径下的所有w3x文件
            w3x_files = find_type_files(input_path, recursive, ("*.w3x",))
            ...
            if w3x_files:
                ...
                for w3x_file in w3x_files:
                    try:
                        logger.info(f"正在处理 w3x 文件: {w3x_file}")
                        
                        # 获取相对于输入路径的相对路径
                        relative_path = os.path.relpath(w3x_file, input_path)
                        relative_dir = os.path.dirname(relative_path)
                        # 从文件路径中提取文件名，不包括扩展名
                        w3x_name = os.path.splitext(os.path.basename(w3x_file))[0]
                        
                        # 创建临时输出目录
                        temp_output_dir = os.path.join(base_output_dir, "output", "w3x_temp", w3x_name)
                        ...
                        logger.error(f"处理 w3x 文件 {w3x_file} 时出错: {e}", exc_info=True)
                        results["errors"] += 1
                        results["details"].append({"file": w3x_file, "status": "错误", "reason": str(e)})
            else:
                logger.warning("没有找到w3x文件!")
        except Exception as e:
            logger.error(f"w3x处理过程中发生错误: {e}", exc_info=True)
            results["errors"] += 1
            results["details"].append({"file": "w3x处理", "status": "错误", "reason": str(e)})
        ...
        if not ini_files:
            ...
            for ini_file in ini_files:
                try:
                    ...
                    ini_type = IniParser.detect_ini_type(file_name) # 检测Ini类型 - 使用类方法而非实例方法
                    
                    if not ini_type:
                        logger.warning(f"无法识别 {ini_file} 的物编类型，跳过。")
                        results["skipped"] += 1
                        results["details"].append({"file": ini_file, "status": "skipped", "reason": "无法识别物编类型"})
                        continue
                    ...
                    results["processed"] += 1
                    results["details"].append({"file": ini_file, "status": "已处理"})
            ...
            if ini_data_dict:
                try:
                    ...
                    success = excel_writer.write_to_excel(ini_data_dict, excel_output_path)
