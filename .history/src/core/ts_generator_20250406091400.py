...
from src.core.excel_parser import ColumnInfo  # 导入日志模块
...
class TSGenerator:
    ...
    def generate_ts_file(self,
        ...
        try:
            ...
            keys = excel_table.get_row(1)

            keys = []
            types = []
            columns_info: List[ColumnInfo] = metadata.get("columns_info")
            for column_info in columns_info:
                keys.append(column_info.key)
                types.append(column_info.type)
            ...
            for i, key in enumerate(keys):
                ...
                # 添加到接口属性定义列表（字段名和类型）
                interface_props.append(f'    "{key}"?: {types[i]};')

            # 生成数据数组项
            data_entries = []

            # 遍历所有数据行，为每行生成对应的TypeScript对象字面量
            for row_idx, rowdata_dict in enumerate(data_rows):
                # 当前行的所有字段键值对
                row_entries = []
                # 获取当前行第一列作为数据项的key值
                ...
                for i, key in enumerate(keys):
                    # 跳过不存在的键和code列
                    if key not in rowdata_dict or key == "code":
                        continue

                    # 获取字段值和推断的类型
                    value = rowdata_dict[key]
                    type = types[i]
