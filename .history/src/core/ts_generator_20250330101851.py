    ...
    def generate_ts_file(self, 
        ...
        try:
            ...
            for i, key in enumerate(keys):
                ...
                # 收集该字段的所有值，用于推断类型
                # 列表推导式：遍历所有数据行，如果行中包含当前键，则提取对应的值
                ...
                if len(types) == 0:  # 如果没有收集到类型（可能全是None）
                    ...
                    field_type = next(iter(types))  # 使用这种类型
                else:
                    # 多种类型情况，使用联合类型
            ...
            if os.path.exists(output_file):
                try:
                    ...
                    self.logger.warning(f"删除已存在文件时出错: {str(remove_error)}，将尝试直接覆盖")
