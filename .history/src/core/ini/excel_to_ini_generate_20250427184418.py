...
class ExcelToIniGenerator:
    ...
    def _generate_modle(self, template_name: str, object_type: str):
        for template_name, template in self.dict_ini_template.items():
            # 只处理unit类相关的特殊模板，且排除已处理的基础单位和瞭望塔
            if ("马甲" in template_name
                    or "无模型" in template_name) and isinstance(template, dict):
                template_id = template.get("id")
                if template_id:
                    # 去除双引号，保持与其他ID一致
                    section_id = str(template_id).strip('"')
                    if section_id not in self.ini_data["unit"]:
                        # 深拷贝模板，避免后续被修改
                        import copy
                        self.ini_data["unit"][section_id] = copy.deepcopy(
                            template)
                        # 记录全局已使用ID（用于冲突报告）
                        if section_id not in self.all_used_ids:
                            self.all_used_ids[section_id] = []
                        self.all_used_ids[section_id].append(
                            ("generated", template_name, 1))
    ...
    def _generate_move_ability(self):
        """
        生成移动技能
        """
    ...
    def merge_ini_data(
        ...
        for obj_type, sections in ini_data.items():
            ...
            for section_id, fields in sections.items():
                # 直接添加新的section，不立即检查冲突
                # 冲突检测将在finish_processing阶段统一进行
    ...
    def process_excel_to_ini(self, excel_data: Dict[str, Dict[str, Any]],
        ...
        try:
            ...
            # 合并INI数据
            self.merge_ini_data(self.ini_data)
        except Exception as e:
            self.logger.error(
                f"处理Excel数据失败: {str(e)}\n{traceback.format_exc()}")
