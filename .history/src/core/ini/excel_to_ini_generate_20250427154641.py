...
class ExcelToIniGenerator:
    ...
    def _process_baize_unit(self, excel_table: ExcelTable, keys: List[str],
        ...
        # === 自动补全所有未专门处理的unit类模板（如马甲单位模版、瞭望塔模版-没有模型） ===
        # 只要dict_ini_template中有、且未被写入result['unit']，就用template['id']为section名，内容为模板本身写入result['unit']。
        # 这样可以保证所有写在提示词上的模板都能被自动支持，后续如有新模板也无需手动加代码。
        for template_name, template in self.dict_ini_template.items():
            ...
            if ("马甲" in template_name
                ...
                if template_id:
                    ...
                    if section_id not in result["unit"]:
                        # 深拷贝模板，避免后续被修改
                        import copy
                        result["unit"][section_id] = copy.deepcopy(template)
                        # 记录全局已使用ID（用于冲突报告）
