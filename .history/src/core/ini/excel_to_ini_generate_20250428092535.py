...
class ExcelToIniGenerator:
    ...
    def _process_general_object(self, excel_table: ExcelTable,
        ...
        for row_idx, row in enumerate(excel_table.data[2:], 2):  # 从第三行开始是数据
            ...
            self.ini_data[obj_type][formatted_id] = obj_dict
    ...
    def _process_baize_item(self, excel_table: ExcelTable, keys: List[str],
        ...
        # 获取使用方式和使用范围列的索引
        key_way = keys.get("使用方式", "无")
        key_range = keys.get("使用范围", 0)
        key_distance = keys.get("使用距离", 0)
        ...
        for row_idx, row in enumerate(excel_table.data[2:], 2):
            ...
            # 处理物品技能
            if key_way >= 0 and key_way < len(row):
                way = str(row[key_way]).strip() if ExcelTable.custom_notna(
                    row[key_way]) else "无"
                ...
                if way == "无目标":
                    ...
                    range_value = row[
                        key_range] if key_range >= 0 and key_range < len(
                            row) and ExcelTable.custom_notna(
                                row[key_range]) else 0
                    if range_value == 0:
                ...
                # 设置使用距离和范围
                if key_distance >= 0 and key_distance < len(
                        row) and ExcelTable.custom_notna(row[key_distance]):
                    ability_dict["Rng"] = self.format_ini_value(
                        row[key_distance])

                if key_range >= 0 and key_range < len(
                        row) and ExcelTable.custom_notna(row[key_range]):
