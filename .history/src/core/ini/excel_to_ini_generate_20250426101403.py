...
class ExcelToIniGenerator:
    ...
    def __init__(self, config_manager: ConfigManager):
        ...
        # 资源文件直接用相对路径，保持与项目其他模块一致
        # INI模板文件路径
        self.ini_template_path = "resource/resource/excel_to_ini/excel_to_ini_template.ini"
        # 编辑器信息映射文件路径
        self.editor_info_map_path = "resource/resource/editor_info_map.ini"
        # 基础命令ID列表文件路径
        self.base_orders_path = "resource/resource/excel_to_ini/基础命令id.txt"

        # 加载模板文件
        self.ini_template_data = self._load_template()

        # 加载编辑器信息映射
        self.editor_info_map = self._load_editor_info_map()

        # 加载基础命令ID列表
        self.order_id_list = self._load_base_orders()
    ...
    def _load_template(self) -> Dict[str, Dict[str, Any]]:
        """
        加载INI模板文件
        ...
        """
        try:
            if not os.path.exists(self.ini_template_path):
                self.logger.error(f"模板文件不存在: {self.ini_template_path}")
                return {}

            return War3IniParser.parse_ini(self.ini_template_path)
        except Exception as e:
            self.logger.error(f"加载模板文件失败: {str(e)}\n{traceback.format_exc()}")
            return {}

    def _load_editor_info_map(self) -> Dict[str, List[str]]:
        """
        加载编辑器信息映射
        ...
        try:
            if not os.path.exists(self.editor_info_map_path):
                self.logger.error(f"编辑器信息映射文件不存在: {self.editor_info_map_path}")
                return {}

            config = ConfigParser()
            config.read(self.editor_info_map_path, encoding='utf-8')
            ...
            for section in config.sections():
                if section in [
                    ...
                    result[section] = [
                        option.lower() for option in config[section]
                    ]
    ...
    def _load_base_orders(self) -> List[str]:
        """
        加载基础命令ID列表
        ...
        """
        try:
            if not os.path.exists(self.base_orders_path):
                self.logger.error(f"基础命令ID文件不存在: {self.base_orders_path}")
                return []

            with open(self.base_orders_path, 'r', encoding='utf-8') as f:
                # 过滤空行和注释行
                return [
    ...
    def get_object_type(self, sheet_name: str,
        ...
        Args:
            sheet_name: Sheet名称
            headers: 表头列表（第二行key值）
            
        Returns:
            Optional[str]: 物编类型，如"unit", "ability", "item"等，如果无法识别则返回None
        """
        # 首先根据sheet名判断
        if sheet_name.lower() in OBJECT_TYPE_MAPPING:
            return OBJECT_TYPE_MAPPING[sheet_name.lower()]

        # 再根据表头中是否包含id和_parent判断
        if "id" in [h.lower() for h in headers
                    ] and "_parent" in [h.lower() for h in headers]:
            # 尝试从_parent值判断类型
            return None  # 这里需要在处理数据时再判断
    ...
    def _process_baize_unit(self, data: List[List[Any]],
        ...
        unit_template = self.ini_template_data.get("基础单位模版", {})
    ...
    def _generate_lookout_towers(self,
        ...
        Args:
            result: 结果字典
        """
        tower_template = self.ini_template_data.get("瞭望塔模版", {})
    ...
    def _generate_basic_abilities(self, result: Dict[str, Dict[str,
        ...
        Args:
            result: 结果字典
        """
        template = self.ini_template_data.get("基础技能模版", {})
        if not template:
    ...
    def _generate_build_abilities(self, result: Dict[str, Dict[str,
        ...
        Args:
            result: 结果字典
        """
        template = self.ini_template_data.get("建造技能模版", {})
        if not template:
    ...
    def _generate_move_ability(self, result: Dict[str, Dict[str, Dict[str,
        ...
        Args:
            result: 结果字典
        """
        template = self.ini_template_data.get("移动技能模版", {})
        if not template:
    ...
    def _generate_inventory_abilities(self,
        ...
        Args:
            result: 结果字典
        """
        template = self.ini_template_data.get("物品栏技能模版", {})
        if not template:
