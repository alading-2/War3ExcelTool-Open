...
class ExcelToIniGenerator:
    ...
    def excel_to_inidata(self, excel_data: Dict[str, Any], file_path: str) -> None:
        """
        根据Excel数据并汇总到INI数据
        ...
        for metadata in excel_data:
            ...
            for type in OBJECT_TYPE_MAPPING.keys():
                if type not in self.ini_data:
                    self.ini_data[type] = {}

            # 白泽框架下特殊处理，只处理名为unit、ability、item的sheet
            if self.baize_frame and is_baize_table and sheet_name.lower() in ("unit", "ability", "item"):
                is_object_table = True

                # 根据物编类型调用相应的处理函数
                if object_type == "unit":
                    self._process_baize_unit(excel_table, keys, file_path, sheet_name)
                elif object_type == "ability":
                    self._process_baize_ability(excel_table, keys, file_path, sheet_name)
                elif object_type == "item":
                    self._process_baize_item(excel_table, keys, file_path, sheet_name)
            ...
            # 通用处理：包含id和_parent的表格
                ...
                is_object_table = True
                self._process_general_object(excel_table, keys, file_path, sheet_name)

            if is_object_table:
                self.list_excel_table_path.append((file_path, sheet_name))

    def _generate_template_object(self, template_name: str, object_type: str):
        """
        生成模版物编
    ...
    def _process_baize_unit(self, excel_table: ExcelTable, keys: List[str], file_path: str, sheet_name: str):
        """
        处理白泽框架下的单位表格
        ...
        for row_idx in range(2, excel_table.shape()[0]):
            ...
            self.all_used_ids[unit_id].append((file_path, sheet_name, row_idx + 1))
    ...
    def _generate_lookout_towers(self):
        ...
        for m in range(1, 5):
            ...
            self.all_used_ids[tower_id].append((file_path, sheet_name, row_idx))

            # 添加到结果字典
            self.ini_data["unit"][tower_id] = tower_dict

    def _process_baize_ability(self, excel_table: ExcelTable, keys: List[str], file_path: str, sheet_name: str):
        """
        处理白泽框架下的技能表格
        ...
        # 生成基础技能
        self._generate_basic_abilities()

        # 生成建造技能
        self._generate_build_abilities()

        # 生成物品栏技能
        self._generate_inventory_abilities()

    def _generate_basic_abilities(self):
        """
        生成基础技能
        """
        ...
        # 技能类型: 无目标/单位目标/点目标
        ability_types = [
            {"prefix": "X", "DataB": "0", "DataC": "1", "targs": None},
            {"prefix": "Y", "DataB": "1", "DataC": "1", "targs": '"notself,ground,structure,air,ward,item,self"'},
            {"prefix": "Z", "DataB": "3", "DataC": "1", "targs": '"notself,ground,structure,air,ward,item,self"'},
        ]
            ...
            for y in range(3):  # 按钮位置y: 0-2
                for x in range(4):  # 按钮位置x: 0-3
                    for ability_type in ability_types:
                        ...
                        # 设置基本属性
                        ability_dict["DataB"] = ability_type["DataB"]  # 数据——目标类型DataB1
                        ability_dict["DataC"] = ability_type["DataC"]  # 数据——选项DataC1
                        if ability_type["targs"]:
                            ability_dict["targs"] = ability_type["targs"]  # 目标允许类型
    ...
    def _generate_build_abilities(self):
        """
        生成建造技能
        """
    ...
    def _process_baize_item(self, excel_table: ExcelTable, keys: List[str], file_path: str, sheet_name: str):
        """
        处理白泽框架下的物品表格
        ...
        for row_idx in range(2, excel_table.shape()[0]):
            ...
            self.all_used_ids[item_id].append((file_path, sheet_name, row_idx + 1))
            ...
            if ability_id not in self.all_used_ids:
                self.all_used_ids[ability_id] = []
            self.all_used_ids[ability_id].append((file_path, sheet_name, row_idx + 1))
            ...
            # 添加到结果字典
            self.ini_data["item"][item_id] = item_dict  # 这个放在后面是因为建造技能

    def _process_general_object(self, excel_table: ExcelTable, keys: List[str], file_path: str, sheet_name: str):
        """
        处理通用物编对象
        ...
        Args:
            ...
            sheet_name: Sheet名称
        """
        for row_idx in range(2, excel_table.shape()[0]):  # 从第三行开始是数据

            obj_id = excel_table.get_value_by_key(row_idx, "id")  # ID
            parent = excel_table.get_value_by_key(row_idx, "_parent")  # 父级

            # 判断物编类型
            obj_type = War3IniParser.detect_ini_type(sheet_name)
            if not obj_type:
                self.logger.error(f"物编表格{file_path}无法根据sheet名:{sheet_name}判断物编ini类型")
                continue

            # 创建对象字典
            obj_dict = {"_parent": parent}
            ...
            for col_idx, key in enumerate(keys):
                ...
                if ExcelTable.custom_notna(value):
                    ...
                    if obj_type in self.dict_editor_info_map:
                        # 检查是否匹配编辑器字段
                        map_dict = self.dict_editor_info_map[obj_type]
                        key_map = None
                        if key in map_dict:
                            key_map = key
                        else:
                        ...
                        if key_map:
                            field_matched = True
                            obj_dict[key_map] = value

            # 记录全局已使用ID（用于冲突报告）
            if obj_id not in self.all_used_ids:
                self.all_used_ids[obj_id] = []
            self.all_used_ids[obj_id].append((file_path, sheet_name, row_idx + 1))
            # 添加到结果字典
            if obj_type not in self.ini_data:
                self.ini_data[obj_type] = {}
            self.ini_data[obj_type][obj_id] = obj_dict
                    ...
                    with open(ini_path, "w", encoding="utf-8") as f:
                        for section_id, fields in sections.items():
                            ...
                            for key, value in fields.items():
                                # 注释
                                comment = self.dict_editor_info_map.get(obj_type, {}).get(key, {})
                                if comment:
                                    f.write(f"# {comment}\n")
                                # 键值对
                                f.write(f"{key}={value}\n")
    ...
    def generate_ts_mapping_files(self, output_dir: str) -> None:
        ...
        try:
            ...
            self.logger.error(f"生成TypeScript映射文件失败: {str(e)}\n{traceback.format_exc()}")

    def process_excel_to_ini(self, excel_data: Dict[str, Dict[str, Any]], file_path: str) -> None:
        """
        处理Excel数据并生成INI文件
        ...
        try:
            ...
            self.logger.error(f"处理Excel数据失败: {str(e)}\n{traceback.format_exc()}")
    ...
    def format_ini_value(self, type: str, value: Any) -> str:
        ...
        if type == "int":
            ...
            # 确保value是字符串类型
            value1 = str(value)
            if (value1.startswith("{") and value1.endswith("}")) or (
                value1.startswith("[=[") and value1.endswith("]=]")
            ):
                # war3物编特殊格式，直接返回
                return value
            else:
        ...
        try:
                ...
                for id, dict2 in dict1.items():
                    ...
                    for key in keys_to_process:
                        if key == "id":  # 跳过id键
                            continue
                        value = dict2[key]
                        type_info = self.dict_editor_info_json_map.get(object_type, {}).get(key, {})
                        value_type = type_info.get("type")
                        if (not value_type) and key == "_parent":
                            value_type = "string"
                        if value_type:  # 确保类型存在
                            formatted_value = self.format_ini_value(value_type, value)
                            self.ini_data[object_type][id][key] = formatted_value
                        else:
                            pass
                            # 如果找不到类型信息，一般是_parent,DataA,DataB,DataC,DataD,DataE,DataF
                            # self.logger.warning(
                            #     f"物编类型 '{object_type}' 的ID '{id}' 中的字段 '{key}' 缺少类型信息，跳过格式化。"
                            # )
                            # self.ini_data[object_type][id][key] = value # 或者保留原值
            ...
            if len_list_excel_table_path > 0:
                ...
                for file_path, sheet_name in self.list_excel_table_path:
                    self.logger.info(f"物编表格{i}. excel路径: {file_path}, 表名: {sheet_name}")
                    i += 1
            else:
