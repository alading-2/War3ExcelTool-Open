    ...
    def generate_ini_from_excel(
        ...
        for metadata in excel_data:
            ...
            :
                ...
                # 根据物编类型调用相应的处理函数
                if object_type == "unit":
                    self._process_baize_unit(excel_table, header_indices, file_path,
                                             sheet_name, result)
                elif object_type == "ability":
                    self._process_baize_ability(excel_table, header_indices,
                                                file_path, sheet_name, result)
                elif object_type == "item":
                    self._process_baize_item(excel_table, header_indices, file_path,
                                             sheet_name, result)

            # 通用处理：包含id和_parent的表格
            elif "id" in header_indices and "_parent" in header_indices:
                self._process_general_object(excel_table, header_indices, file_path,
                                             sheet_name, result)
        ...
        for row_idx, row in enumerate(data[2:], 2):
            ...
            for header, idx in header_indices.items():
                if idx < len(row) and row[idx] is not None:
                    # 检查是否匹配编辑器字段
                    for field in self.dict_editor_info_map.get("unit", []):
                        if header.lower() == field.lower():
                            # 特殊字段处理
