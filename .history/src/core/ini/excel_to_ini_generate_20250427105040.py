...
class ExcelToIniGenerator:
    ...
    def _generate_lookout_towers(self,
        ...
        for m in range(1, 5):
            ...
            if file_path not in self.all_used_ids:
                self.all_used_ids[file_path] = {}
            if sheet_name not in self.all_used_ids[file_path]:
                self.all_used_ids[file_path][sheet_name] = []
            self.all_used_ids[file_path][sheet_name].append(
                (tower_id, row_idx))
    ...
    def _generate_move_ability(self, result: Dict[str, Dict[str, Dict[str,
        ...
        if sheet_name not in self.all_used_ids[file_path]:
            self.all_used_ids[file_path][sheet_name] = []
        self.all_used_ids[file_path][sheet_name].append((ability_id, row_idx))
