...
class ExcelToIniGenerator:
    ...
    def format_ini_value(self, type: str,
                         value: Any) -> Union[int, float, str]:
        """
        格式化INI值，确保字符串有双引号
        
        Args:
            type: 目标类型 (int, float, string)
            value: 原始值
            
        Returns:
            Union[int, float, str]: 格式化后的值，类型与目标类型一致
        """
        ...
        try:
            if type == "int":
                # 先尝试转为float再转int，处理'1.0'之类的情况
                return int(float(value))
            elif type == "float":
                return float(value)
            elif type == "string":
                # 确保 value 是字符串类型再进行判断
                str_value = str(value)
                # War3 物编特殊格式检查
                if (str_value.startswith("{") and str_value.endswith("}")) or \
                   (str_value.startswith("[=[") and str_value.endswith("]=]")) or \
                   (str_value.startswith('"') and str_value.endswith('"')): # 如果本身带引号
                    return str_value
                else:
                    # 普通字符串：去除首尾可能存在的引号，然后添加双引号
                    cleaned_value = str_value.strip('"')
                    return f'"{cleaned_value}"'
            else:
                # 对于未知类型，可以选择返回原值或记录警告
                self.logger.warning(f"未知的格式化类型 '{type}'，返回原始值: {value}")
                return str(value)  # 保证返回的是字符串

        except ValueError as e:
            self.logger.error(f"格式化值时出错: 类型='{type}', 值='{value}'. 错误: {e}")
            # 根据类型返回一个安全的默认值
            if type == "int":
                return 0
            elif type == "float":
