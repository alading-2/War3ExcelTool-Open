...
class ExcelToIniGenerator:
    ...
    def generate_ini_from_excel(self, excel_data: Dict[str, Any],
                                file_path: str) -> None:
        """
        根据Excel数据生成INI数据
        ...
        for metadata in excel_data:
            ...
            if self.baize_frame and is_baize_table and sheet_name.lower() in (
                ...
                for template_name, object_type in template:
                    self._generate_template_object(template_name, object_type)
    ...
    def _process_baize_item(self, excel_table: ExcelTable, keys: List[str],
        ...
        for row_idx, row in enumerate(excel_table.data[2:], 2):
            ...
            for col_idx, key in enumerate(keys):
                ...
                if ExcelTable.custom_notna(value):
                    # 检查是否匹配编辑器字段
                    for map_dict in self.dict_editor_info_map.get("item", []):
                        # 检查是否匹配编辑器字段
                        key_map = None
                        if key in map_dict:
                            key_map = key
            ...
            for col_idx, key in enumerate(keys):
                ...
                if ExcelTable.custom_notna(value):
                    ...
                    if obj_type in self.dict_editor_info_map:
                        for map_dict in self.dict_editor_info_map[obj_type]:
                            # 检查是否匹配编辑器字段
                            key_map = None
                            if key in map_dict:
                                key_map = key
                    ...
                    if not field_matched and row[col_idx] is not None:
                        obj_dict[key] = self.format_ini_value(row[col_idx])
    ...
    def process_excel_to_ini(self, excel_data: Dict[str, Dict[str, Any]],
        ...
        """
        try:
            # 生成INI数据，直接更新self.ini_data
            self.generate_ini_from_excel(excel_data, file_path)
        except Exception as e:
            self.logger.error(
                f"处理Excel数据失败: {str(e)}\n{traceback.format_exc()}")
