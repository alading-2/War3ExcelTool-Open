...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Excel到TypeScript转换工具 - 支持JSON配置",
    ...
    # 可选的配置文件路径参数
    parser.add_argument("-c", "--config", help="指定配置文件路径，默认使用src/setting.json")
...
def main():
    ...
    if is_cli_mode:
        ...
        config_manager = JsonConfigManager(args.config if hasattr(args, 'config') else None)
