...
for key, data in dict2.items():
    ...
    if "displayname" in keys and "field" in keys:
        ...
        # 生成.json
        for object_type, data1 in dict1.items():
            if displayname in data1.keys():
                ...
                json_dict[object_type][field]["comment"] = data1[displayname]
...
for object_type, data1 in json_dict.items():
    path4 = os.path.join(r"resource\resource", f"{object_type}.json")
    for field, data2 in data1.items():
        if field in order_dict3:
            order_dict3[field] = object_type
