...
def find_excel_files(input_dir: str, recursive: bool = False) -> List[str]:
    """
    在指定目录中查找Excel文件
    ...
    # 构建文件路径模式
    # os.path.join 用于连接目录路径和文件名模式
    # "**" 表示任意层级的子目录（只在recursive=True时使用）
    # "*.xlsx" 表示所有扩展名为.xlsx的文件
    pattern = os.path.join(input_dir, "**" if recursive else "", "*.xlsx")

    # 使用glob.glob函数查找匹配模式的所有文件
    # recursive参数控制是否递归搜索子目录
    excel_files = glob.glob(pattern, recursive=recursive)
    # 移除~$*.xlsx，即临时文件
    for file in excel_files:
        if re.findall(r"~$\w+.xlsx", file):
...
def get_excel_name(file_path: str) -> str:
    ...
    提取文件的基本名称，不包括路径和扩展名。
    例如，从"C:/data/example.xlsx"提取"example"。

    Args:
        file_path: Excel文件的完整路径或相对路径

    Returns:
        文件名（不含路径和扩展名）

    Example:
        >>> name = get_excel_name("C:/data/example.xlsx")
        >>> print(name)
        'example'
    """
