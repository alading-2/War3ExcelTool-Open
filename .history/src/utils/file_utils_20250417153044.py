...
def find_excel_files(input_dir: str, recursive: bool = False,extension:Optional(Tuple)=("*.xls", "*.xlsx")) -> List[str]:
    """
    在指定目录中查找Excel文件
    ...
    # 过滤掉临时文件~$
    # os.path.normpath 规范化每个文件路径，比如把'else/Test/excel\\test\\xls格式.xls'都替换成\
    excel_files = [os.path.normpath(f) for f in excel_files if not os.path.basename(f).startswith("~$")]
