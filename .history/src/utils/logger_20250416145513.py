...
# %(name)s：日志记录器名称
# %(message)s：日志消息内容
LOG_FORMAT = "[时间]：%(asctime)s ，[等级]：%(levelname)s，[name]: %(name)s，[line]：%(lineno)d，[message]: %(message)s"

# 彩色日志格式，用于控制台输出
# 添加了colorlog特有的颜色标记：log_color和reset
COLOR_LOG_FORMAT = "%(log_color)s [时间]：%(asctime)s ，[等级]：%(levelname)s，[name]: %(name)s，[line]：%(lineno)d，[message]: %(message)s %(reset)s"
...
def setup_logger(log_dir: Optional[str] = None, log_level: str = "INFO") -> logging.Logger:
    ...
    # log_dir='C:\\Users\\29701\\.war3exceltool\\logs'
    if log_dir:
        # 确保日志目录存在，如果不存在则创建
