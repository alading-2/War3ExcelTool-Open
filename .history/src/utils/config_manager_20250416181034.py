...
class ConfigManager:
    ...
    DEFAULT_CONFIG = {
        # Paths
        "input_dir": "",  # Excel 输入目录/文件
        "output_dir": "",  # 基础输出目录
        "ini_input_path": "", # Ini 输入目录/文件
        "template_path": "", # TS 模板文件路径 (如果需要)
        # Options
        "recursive": False,  # 是否递归处理输入目录
        "batch_size": 10,  # 批处理大小 (可能不再直接使用，取决于 processor 实现)
        "log_level": "INFO",  # 日志级别
        # Conversion Flags
        "convert_to_ts": True, # 默认启用 Excel -> TS
