...
from src.utils.json_config import JsonConfigManager  # 使用配置管理类
...
def parse_args():
    ...
    parser.add_argument("--w3x2lni-path", help="设置w3x2lni工具的路径")
    parser.add_argument("--parse-w3x", action="store_true", help="是否解析w3x文件")
    parser.add_argument("--sort-by-alpha", action="store_true", help="生成表格是否按字母排序")
    parser.add_argument("--font-size", type=int, default=14, help="设置字体大小")
...
def main():
    ...
    if is_cli_mode:
        ...
        if args.w3x2lni_path:
            config["w3x2lni_path"] = os.path.normpath(args.w3x2lni_path)
        if args.parse_w3x:
            config["parse_w3x"] = args.parse_w3x
        if args.sort_by_alpha:
            config["sort_by_alpha"] = args.sort_by_alpha
        if args.font_size:
