...
exe = EXE(
    ...
    splash.binaries,
    [],           # 空列表（旧版兼容）
    name='War3ExcelTool',          # 输出文件名（不带扩展名）
    workpath='package/build',         # 构建缓存目录
    distpath='package/War3ExcelTool',  # 输出目录及名称
    ...
    icon=r'resource\picture\文件图标.ico',            # 可执行文件的图标路径
)