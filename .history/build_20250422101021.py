...
def close_running_exe(exe_name=f"{project_info.name}.exe"):
    """
    检查并关闭正在运行的同名exe
...
def build_executable():
    ...
    cmd = [
        ...
        "--add-data=src/favicon.ico;src",
        "--icon=src/favicon.ico",
        "--splash=splash.png",
        f"--distpath=package\\{project_info.name}",
        "War3ExcelTool.spec",
    ]
    ...
    # 复制配置文件和其他必要的文件到输出目录
    dist_dir = f"package\\{project_info.name}"
    ...
    # 复制配置文件
    if os.path.exists("src/config.cfg"):
        shutil.copy("src/config.cfg", dist_dir)
...
def main():
    """主函数"""
    # 检查并关闭正在运行的程序
    close_running_exe(f"{project_info.name}.exe")
