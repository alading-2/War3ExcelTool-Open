...
class TestPretreatment(unittest.TestCase):
    ...
    def test_extract_multiple_directives(self):
        """测试从单元格中提取多个预处理指令"""
        # 创建包含多个指令的测试DataFrame
        data = [
            ["说明 #default 0 #path", "Normal Header"],
            ["Icon Path", "Description"],
            ["sword\\icon.blp", "A sword"],
            ["axe\\icon.blp", "An axe"]
        ]
        df = pd.DataFrame(data)
        ...
        # 验证提取结果
        self.assertEqual(len(column_preprocessors), 1)
        self.assertEqual(directive_rows, {0})
        
        # 验证第一列包含两个预处理规则
        self.assertIn((0, "Icon Path"), column_preprocessors)
        self.assertEqual(column_preprocessors[(0, "Icon Path")]["#default"], 0)
        self.assertTrue(column_preprocessors[(0, "Icon Path")]["#path"])

    def test_extract_directives_at_end(self):
        """测试从单元格末尾提取预处理指令"""
        # 创建包含末尾指令的测试DataFrame
        data = [
            ["图标路径信息 #path", "默认攻击力 #default 10"],
            ["Icon Path", "Attack"],
            ["sword\\icon.blp", None],
            ["axe\\icon.blp", 20]
        ]
        ...
        # 验证第二列的预处理规则
        self.assertIn((1, "Attack"), column_preprocessors)
        self.assertEqual(column_preprocessors[(1, "Attack")]["#default"], 10)
