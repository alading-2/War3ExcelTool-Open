...
class TestExcelTable(unittest.TestCase):
    ...
    def test_initialization(self):
        ...
        # 检查注释是否正确存储
        self.assertEqual(self.excel_table.column_info["key1"].comment,
                         "Comment1")
        self.assertEqual(self.excel_table.column_info["key5"].comment,
                         "Comment5")
    ...
    def test_get_cell(self):
        ...
        with self.assertRaises(IndexError):
            self.excel_table.get_cell(3, 0)  # 行索引过高
        with self.assertRaises(IndexError):
            self.excel_table.get_cell(0, 4)  # 列索引过高
